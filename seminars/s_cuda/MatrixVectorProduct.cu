...
#define	d_A(i,j) 	d_A[ (i) + ((j)*(m)) ]
#define	d_x(i) 		d_x[ (i) ]
#define	d_y(i) 		d_y[ (i) ]

#define	BLOCKSIZE	512

int matrix_vectorc( unsigned int m, unsigned int n, float *A, float *x, float *y ) {
  unsigned int i, j;
...
__global__ void compute_kernel( unsigned int m, unsigned int n, float *d_A, float *d_x, float *d_y ) {
  ...
  while(tid_x < n)
  {
    temp += (d_x[tid_x] * d_A[tid_x + tid_y*n]);
    tid_x += BLOCKSIZE;
  }
